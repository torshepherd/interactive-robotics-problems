...
# Reading state.tmp, writing ref.tmp
with open('state.tmp', 'wb') as f_s, open('ref.tmp', 'wb') as f_r, open("input.tmp", "wb") as f_i:
    f_s.write(struct.pack('f', 0.0) * 4 + b'\n\n\n')
    f_r.write(struct.pack('f', 0.0))
    f_i.write(struct.pack('f', 0.0))
...
with open("state.tmp", "r+b") as f_s, open("ref.tmp", "r+b") as f_r, open("input.tmp", "r+b") as f_i:
    ...
    while not done:
        ...
        while standing:
            ...
            try:
                inputs = struct.unpack('f', input_mm.readline())[0]
            except:
            ...
            # Write states
            state_mm[:] = struct.pack(
                'f', float(states[0, 0])) + b'\n' + struct.pack(
                    'f', float(states[1, 0])) + b'\n' + struct.pack(
                    'f', float(states[2, 0])) + b'\n' + struct.pack(
                    'f', float(states[3, 0]))
